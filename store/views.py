...
from .models import Customer, OrderItem, Product, Collection, Review, Cart, CartItem, Order
...
class ProductViewSet(ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        if OrderItem.objects.filter(product_id=kwargs['pk']).count() > 0:
            return Response({'error': 'Product cannot be deleted because its associated to another item'}, status.HTTP_405_METHOD_NOT_ALLOWED)
